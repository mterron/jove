...
var IDLE_TIME = 10000 * 6;           // Time before idle becomes true again

var FOV = 45;                       // Camera field of view
var NEAR = 1;                       // Camera near
var FAR = 150000;                   // Draw distance

var VISIBLE = true; // Use the visibility API to avoid creating a ton of data when the user is not looking
  ...
  'atmosphere' : {
    ...
    fragmentShader: [
      ...
      'void main() {',
        ...
        'gl_FragColor = vec4( 0.7, 0.7, 1.0, 0.9 ) * intensity;',
      '}'
    ].join('\n')
  }
};
...
/*
var urls = [
  'assets/Skybox_PositiveX.jpg',
  'assets/Skybox_NegativeX.jpg',
  'assets/Skybox_PositiveY.jpg',
  'assets/Skybox_NegativeY.jpg',
  'assets/Skybox_PositiveZ.jpg',
  'assets/Skybox_NegativeZ.jpg'
];
...
function addEarth() {
  ...
  uniforms['texture'].value = THREE.ImageUtils.loadTexture('assets/2_no_clouds_8k_shrunk.jpg');
...
//  mesh.scale.set(1.1, 1.1, 1.1);
  mesh.scale.set(1.125, 1.125, 1.125);
  scene.add(mesh);
...
function latLonToVector3(lat, lon) {
  ...
  lon = lon - 180;
    ...
    var phi = (90 - lat) * Math.PI / 180; //in radians
    var theta = (180 - lon) * Math.PI / 180; //in radians
...
function bezierCurveBetween(startVec3, endVec3) {
 //calculate control points
  ...
  var anchorHeight = RADIUS + distanceBetweenPoints * 0.4;
...
// Stop drawing points that have been around too long
  ...
  //  var subLatLon = { lat: subscribes[i][0], lon: subscribes[i][1] };
    var subLatLon = { lat: subscribes[0], lon: subscribes[1] };
...
var overlay;
function addOverlay() {
  var spGeo = new THREE.SphereGeometry(RADIUS + 5, 50, 50);
